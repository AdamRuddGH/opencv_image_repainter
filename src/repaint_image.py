...
def repaint(constraint_angle=False,upscale=1.,batchsize=16, brush='random'):  #adam edited this to add brush param
    starttime = time.time()
...
def paint_one(x,y,brushname='random',angle=-1.,minrad=10,maxrad=60):
    ...
    def paint_final_w(color,angle,nr):
        ...
        rb.compose(canvas,brush,x=x,y=y,rad=radius,srad=srad,angle=angle,color=color,usefloat=True,useoil=False,lock=canvaslock) # was useoil = True
...
def putstrokes(howmany, minrad=10, maxrad=50,brushname='random'):  #this will error if no valie is included but it should be passed through in r()
    ...
    def pcasync(tup):
        ...
        b,key = rb.get_brush(key=brushname) # get a random brush if not supplied in parent function
        return paint_one(x,y,brushname=key,minrad=minrad,maxrad=maxrad,angle=angle) #num of epoch #was min 10 max 50 - moved to the parent function
...
def r(epoch=1,minsize=10,maxsize=50,brushname='random', numstrokes=512):  #control the size per r
    ...
    for i in range(epoch):
        ...
        for k in range(loopfor):
            ...
            res = putstrokes(paranum,minsize,maxsize,brushname)
            #res is a map of results
