...
# the brush process
def compose(orig,brush,x,y,rad,srad,angle,color,usefloat=False,useoil=True,lock=None):
    ...
    if useoil:
        ...
        # some noise?
        gbmix += np.random.normal(loc=0.15,scale=.2,size=gbmix.shape)  # adam says no noise
